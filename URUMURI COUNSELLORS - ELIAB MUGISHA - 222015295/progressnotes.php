...
<header>
    ...
    <ul> <img src="logo platform.JFIF" alt="Logo" height="50" width="50">
...
<section>
    ...
    <form method="post" action="progressnotes.php">
        ...
        <label for="NoteContent">NoteContent:</label>
        <input type="text" id="NoteContent" name="NoteContent" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO progressnotes (NoteID, SessionID, NoteContent) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $NID, $SID, $NC);
        $NID = $_POST['NoteID'];
        $SID = $_POST['SessionID'];
    ...
    ?>
    <!-- Data Display Table -->
    <center><h2>Table of progressnotes</h2></center>
    <table>
        <tr>
            <th>NoteID</th>
            <th>SessionID</th>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    <td>{$row["NoteID"]}</td>
                    <td>{$row["SessionID"]}</td>
                    <td>{$row["NoteContent"]}</td> 
                     
                    <td><a href='delete_progressnotes.php?NoteID={$row["NoteID"]}'>Delete</a></td> 
                    <td><a href='update_progressnotes.php?NoteID={$row["NoteID"]}'>Update</a></td> 
                </tr>";
            }
        } else {
