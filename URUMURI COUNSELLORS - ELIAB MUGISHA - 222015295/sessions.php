...
<head>
    ...
    <style>
        ...
        .dropdown-content a:hover {
            background-color: olive; /* Updated hover background color */
        }
    </style>
</head>
<body>
...
<section>
    ...
    <form method="post" action="sessions.php">
        ...
        <input type="text" id="SessionNotes" name="SessionNotes" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO sessions (SessionID, CounselorID, ClientID,StartTime,EndTime,SessionNotes) VALUES (?, ?, ?,?,?,?)");
        $stmt->bind_param("isssss",  $SID,$COID, $CLID,$STIME,$ETIME,$SNOTE);
        $SID = $_POST['SessionID'];
        $COID = $_POST['CounselorID'];
        $CLID = $_POST['ClientID'];
        $STIME = $_POST['StartTime'];
        $ETIME = $_POST['EndTime'];
         $SNOTE = $_POST['SessionNotes'];
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td><a href='delete_sessions.php?SessionID={$row["SessionID"]}'>Delete</a></td> 
                    <td><a href='update_sessions.php?SessionID={$row["SessionID"]}'>Update</a></td> 
                </tr>";
            }
        } else {
