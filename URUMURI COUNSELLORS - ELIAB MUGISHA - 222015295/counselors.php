...
<section>
    ...
    <form method="post" action="counselors.php">
        ...
        <input type="number" id="LicenseNumber" name="LicenseNumber" required><br><br>
         <label for="Education">Education:</label>
        <input type="date" id="Education" name="Education" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO counselors (CounselorID, Experience, LicenseNumber,Education) VALUES (?, ?, ?,?)");
        $stmt->bind_param("isss",  $CID, $E,$LNUMBER,$EDU);
    ...
    ?>
    <!-- Data Display Table -->
    <center><h2>Table of counselors</h2></center>
    <table>
        <tr>
            <th>CounselorID</th>
            <th>Experience</th>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td><a href='delete_counselors.php?CounselorID={$row["CounselorID"]}'>Delete</a></td> 
                    <td><a href='update_counselors.php?CounselorID={$row["CounselorID"]}'>Update</a></td> 
                </tr>";
            }
        } else {
