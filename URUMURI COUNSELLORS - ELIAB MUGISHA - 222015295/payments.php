...
<section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO payments (PaymentID, Amount, PaymentDate,PaymentMethod) VALUES (?, ?, ?,?)");
        $stmt->bind_param("isss",  $PID, $A,$PDATE,$PM);
        $PID = $_POST['PaymentID'];
        $A = $_POST['Amount'];
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td><a href='delete_payments.php?PaymentID={$row["PaymentID"]}'>Delete</a></td> 
                    <td><a href='update_payments.php?PaymentID={$row["PaymentID"]}'>Update</a></td> 
                </tr>";
            }
        } else {
