...
<section>
    ...
    <form method="post" action="clients.php">
        ...
        <input type="text" id="RelationshipLength" name="RelationshipLength" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO clients (ClientID, RelationshipStatus,RelationshipLength) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $CLID, $RE, $RL);
        $CLID = $_POST['ClientID'];
        $RE = $_POST['RelationshipStatus'];
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    <td>{$row["ClientID"]}</td>
                    <td>{$row["RelationshipStatus"]}</td>
                    <td>{$row["RelationshipLength"]}</td> 
                     
                    <td><a href='delete_clients.php?ClientID={$row["ClientID"]}'>Delete</a></td> 
                    <td><a href='update_clients.php?ClientID={$row["ClientID"]}'>Update</a></td> 
                </tr>";
            }
        } else {
