...
<section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO profiles (ProfileID, FullName, Gender,Age,Location) VALUES (?, ?, ?,?,?)");
        $stmt->bind_param("issss", $PID, $FNAME, $G,$A,$L);
        $PID = $_POST['ProfileID'];
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td>{$row["Gender"]}</td> 
                     <td>{$row["Age"]}</td>
                    <td>{$row["Location"]}</td> 
                    <td><a href='delete_profiles.php?ProfileID={$row["ProfileID"]}'>Delete</a></td> 
                    <td><a href='update_profiles.php?ProfileID={$row["ProfileID"]}'>Update</a></td> 
                </tr>";
            }
        } else {
